...
class Log {
	...
	public static function mostrarRegistroDeAccionesByIdUsuario($idUsuario){
		$sql = "Select l.idlog,id_usuario,l.nombre_usuario,l.id_rol,r.nombre_rol,l.accion,l.descripcion,l.fecha,p.ci,p.nombre_per,p.paterno
		From log l, usuario u, rol r, persona p
		Where l.id_usuario = u.id_usu and u.id_rol = r.id_rol and u.ci_persona = p.ci and u.id_usu = :idUsu
        order by l.fecha desc";

		$con = Database::getConexion(); 
		$stmt = $con->prepare($sql);
        $stmt->bindValue(":idUsu", $idUsuario,PDO::PARAM_INT);
